...
from feature_new import FeatureExistsError, optimized_make_feature, make_feature, optimized_make_feature2
...
if __name__ == '__main__':
	...
	if argument.trade_cd=='t':
		argument.model_path = os.path.join('./model', 'store')
		argument.model_info = {
			full_feature_model_name: os.path.join(argument.model_path, full_feature_model_name + '.model'),
			sale_feature_model_name: os.path.join(argument.model_path, sale_feature_model_name + '.model'),
			trade_feature_model_name: os.path.join(argument.model_path, trade_feature_model_name + '.model')
		}
	else:
		argument.model_path = os.path.join('./model', 'store_rent')
		argument.model_info = {
			full_feature_model_name: os.path.join(argument.model_path, full_feature_model_name + '.model'),
			sale_feature_model_name: os.path.join(argument.model_path, sale_feature_model_name + '.model'),
			trade_feature_model_name: os.path.join(argument.model_path, trade_feature_model_name + '.model')
	...
	# -------------------------------------------------------------------------------------------------------------- #
		...
		if argument.db_inject:
			...
			for detail_pk in _result_df.apt_detail_pk:
				db_to_frame = pd.read_sql_query(f"SELECT * from predicate_price_ WHERE apt_detail_pk={detail_pk};",settings.cnx)
