...
class GinAptQuery(object):
	...
	def get_trade_price_optimize(apt_detail_pk, trade_cd):
		if trade_cd == 't':
			...
			# TODO : 코드 변경 필요!!
			query = """
				SELECT pk_apt_rent, pk_apt_detail, 
                       CAST(CONCAT(year, '-', LPAD(mon,2,'0'), '-', LPAD(day,2,'0')) AS DATETIME), 
                       CAST(floor AS SIGNED) AS floor, apt_detail.extent, deposit / apt_detail.extent AS price  
                FROM apt_rent
                INNER JOIN apt_detail
                      ON apt_detail.idx = apt_rent.pk_apt_detail
                WHERE pk_apt_detail in (%s)
                  AND EXCEPT_YN='n'
                ORDER BY deal_ymd, pk_apt_rent;
            """ % (apt_detail_pk,)
		# print(query)
		cursor.execute(query)
		return cursor
...
# print(len(predpk))
#
# notpred = list(sorted(set(allpk).difference(predpk)))
# print(len(notpred))
#
# with open('model/pk_list_rent.txt', 'w') as f:
# 	for item in notpred:
# 		f.write("%s\n" % item)
...
#     @staticmethod
#     def bbb():
#         query = """SELECT distinct(apt_detail_pk) FROM GIN.predicate_price_ where trade_cd="t";"""
#         cursor.execute(query)
#         return cursor
#
# #
...
#
#
# with open('model/pk_list.txt', 'w') as f:
#     for item in notpred:
#         f.write("%s\n" % item)
